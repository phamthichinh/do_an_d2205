...
public class Board : MonoBehaviour
{
    ...
    private void DestroyMatchedGemAt(Vector2Int pos) // Phá hủy đá
    {
        if (allGems[pos.x, pos.y] != null)
        {
            if (allGems[pos.x, pos.y].isMatched)
            {
                ...
                {
                    SFXManager.instance.PlayExplore();
                }
                if (allGems[pos.x, pos.y].type == Gem.GemType.dice)
                {
                    SFXManager.instance.PlayDiceBreak();
                }
                else
                {
    ...
    public void DestroyMatches() // phá hủy match
    {
    ...
    {
        if (currentState != Board.BoardState.wait)
        {
